...
Compares multiple concurrency levels. Run on the pod itself for accurate GPU metrics.
...
Usage examples:
    ...
    python test_pod.py --url https://<pod-id>-8080.proxy.runpod.net

    # On the pod (localhost), custom concurrency ladder
    python test_pod.py --url http://localhost:8080 --levels 1 4 8 16 31

    # Skip sequential baseline
    python test_pod.py --url http://localhost:8080 --levels 4 8 --no-save
"""
import argparse
import base64
import io
...
DEFAULT_URL     = "https://eiqhnj3vhd6bkc-8080.proxy.runpod.net"
DEFAULT_CHUNKS  = 8
DEFAULT_TIMEOUT = 300
DEFAULT_PDF_URL = (
...
def parse_args():
    p = argparse.ArgumentParser(description="PaddleOCR Section 4 pod benchmark")
    p.add_argument("--url", default=DEFAULT_URL,
                   help="Pod base URL, e.g. https://<id>-8080.proxy.runpod.net  "
                        "(default: %(default)s)")
    ...
    p.add_argument("--levels", type=int, nargs="+",
                   help="Concurrency levels to test, e.g. --levels 1 4 8 16  "
                        "(default: 1, 4, <chunks>)")
...
def main():
    ...
    for level in levels:
        label = (
            "SEQUENTIAL (Section 4 baseline)" if level == 1
            else f"{level} CONCURRENT"
        )
        total_time, total_pages, results = run_test(
            chunks, level, label, gateway_url, args.timeout
        )
