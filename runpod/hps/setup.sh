...
(
  set -euo pipefail
  if [ -f "$WORKSPACE/hps/server/pipeline_config.yaml" ] && \
     [ -f "$WORKSPACE/hps/server/model_repo/layout-parsing/config.pbtxt" ]; then
    log "[A] HPS SDK already on volume — skipping download"
  else
...
print("[A] pipeline_config.yaml: backend set to vllm-server, max_concurrency=32")
PYEOF
...
(
  ...
  # Skip if already installed (e.g. using pre-built Docker image)
  if $PADDLE_PY -c "import vllm, paddleocr, flash_attn" 2>/dev/null; then
    log "[B] Python deps already installed — skipping"
    exit 0
  fi
...
if [ -f $WORKSPACE/.venv_gateway/bin/uvicorn ]; then
  log "Gateway venv already on volume — skipping"
else
