...
Compares multiple concurrency levels. Run on the pod for accurate GPU metrics.

Install deps:
    pip install pypdf requests

Usage examples:
    # Run on the pod (real GPU metrics)
    python test_benchmark.py

    # Custom concurrency ladder
    python test_benchmark.py --url http://localhost:8080 --levels 1 4 8 16 32

    # Split into more chunks for higher concurrency tests (max = total pages)
    python test_benchmark.py --chunks 31 --levels 1 8 16 31

    # From outside (note: RunPod proxy has ~140s timeout — use TCP port for long tests)
    python test_benchmark.py --url http://<pod-ip>:<tcp-port> --levels 1 4 8
"""
import argparse
import base64
import io
...
DEFAULT_URL     = "http://localhost:8080/layout-parsing"
DEFAULT_CHUNKS  = 10
DEFAULT_TIMEOUT = 300
DEFAULT_PDF_URL = (
...
def parse_args():
    ...
    p.add_argument("--chunks", type=int, default=DEFAULT_CHUNKS,
                   help="Number of PDF chunks (default: %(default)s)")
...
def query(chunk_bytes: bytes, gateway_url: str, timeout: int, t0: float) -> tuple:
    ...
    r = requests.post(
        gateway_url,
        json={"file": base64.b64encode(chunk_bytes).decode(), "fileType": 0, "visualize": False},
        timeout=timeout,
    )
    r.raise_for_status()
...
def print_gpu_summary(samples: list):
    ...
    print(f"  GPU util : avg={sum(utils)/len(utils):.0f}%  min={min(utils)}%  max={max(utils)}%")
...
def main():
    ...
    for level in levels:
        label = "SEQUENTIAL (baseline)" if level == 1 else f"{level} CONCURRENT"
