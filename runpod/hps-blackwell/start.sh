...
# paddleocr 3.5.0 requires `install_genai_server_deps vllm` before genai_server
# is registered as a CLI subcommand. Start vLLM directly instead — the HPS
# pipeline uses vllm-server backend (HTTP to localhost:8118) regardless.
MODEL_DIR=${PADDLEOCR_VL_MODEL_DIR:-/root/.paddlex/official_models/PaddleOCR-VL}
VLLM_ARGS=(
    --model "$MODEL_DIR"
    --host 0.0.0.0
    --port "$VLLM_PORT"
...
echo "[start] vLLM args: ${VLLM_ARGS[*]}"
$PADDLE_PY -m vllm.entrypoints.openai.api_server \
    "${VLLM_ARGS[@]}" \
    2>&1 | sed -u 's/^/[vllm] /' &
VLLM_PID=$!
