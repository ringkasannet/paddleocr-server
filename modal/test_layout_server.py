"""Test script for the local layout server.

Usage:
    python modal/test_layout_local.py document.pdf
    python modal/test_layout_local.py document.pdf --repeat 3
    python modal/test_layout_local.py document.pdf --concurrent 4
    python modal/test_layout_local.py document.pdf --repeat 2 --concurrent 4
    python modal/test_layout_local.py document.pdf --no-regions
    python modal/test_layout_local.py document.pdf --timeline     # show ASCII bars
    python modal/test_layout_local.py document.pdf --save
    python modal/test_layout_local.py image.jpg --image
    python modal/test_layout_local.py document.pdf --endpoint http://localhost:8000
"""
...
COLD_THRESHOLD = 5.0   # queued_s above this → cold start (kept for compat if pointed at Modal)
...
def send(endpoint: str, file_bytes: bytes, file_type: int, dpi: int,
    ...
    try:
        ...
        upload_s       = round(t_first_byte - t0 - encode_s, 3)
...
def print_summary(results: list[tuple[str, float, dict]], round_elapsed: float | None = None):
    ...
    print(f"{'═' * 72}")
    print(f"  {'Label':<20}  {'srv_wall':>8}  {'ttfb':>7}  {'net_s':>6}  {'download':>8}  {'render':>6}  {'queue':>6}  {'detect':>7}  {'text':>5}  {'pages':>5}")
    print(f"  {'─'*20}  {'─'*8}  {'─'*7}  {'─'*6}  {'─'*8}  {'─'*6}  {'─'*6}  {'─'*7}  {'─'*5}  {'─'*5}")
    ...
    for label, wall, data in results:
        ...
        print(f"  {label:<20}  {_s(srv_wall,8)}  {_s(ttfb_s,7)}  {_s(net_s,6)}  {_s(download_s,8)}  {_s(render_s)}  {_s(queued_s)}   {_s(detect_s)}  {_s(text_s,5)}  {str(pages):>5}")
    ...
    if n > 1:
        ...
        print(f"  {'TOTAL':<20}  {_s(total_srv,8)}  {_s(total_ttfb,7)}  {_s(total_net,6)}  {_s(total_download,8)}  {_s(total_render)}  {_s(total_queue)}   {_s(total_detect)}  {_s(total_text,5)}  {int(total_pages):>5}")
