"""Test script for the deployed GLM-OCR pipeline endpoint.

Usage:
    python modal/test_glm_ocr_pipeline.py document.pdf
    python modal/test_glm_ocr_pipeline.py document.pdf --pages 0 1 2
    python modal/test_glm_ocr_pipeline.py document.pdf --concurrent 3
    python modal/test_glm_ocr_pipeline.py document.pdf --repeat 2 --concurrent 3
    python modal/test_glm_ocr_pipeline.py document.pdf --timeline
    python modal/test_glm_ocr_pipeline.py document.pdf --save
    python modal/test_glm_ocr_pipeline.py document.pdf --endpoint https://your-url.modal.run
"""
    ...
    for r in range(args.repeat):
        ...
        for label, wall, data in results:
            ...
            if (args.save or single) and "error" not in data:
                ...
                path    = f"{stem}_pipeline_{label.replace(' ','_')}_{ts}"
                out_dir = Path(path)
                out_dir.mkdir(exist_ok=True)
                # strip image_b64 from JSON (save as files instead)
